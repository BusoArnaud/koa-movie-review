...
describe('API', () => {
  ...
  test('should respond on home', async () => {
    await supertest(server).get('/').expect(200, { you: '<- are here !' });
  });

  test('should get empty movies', async () => {
  ...
  test('should return the token when login', async () => {
  ...
  test('should return 403 if wrong password', async () => {
    await db.knex.table('users').insert([
      {
        user_id: '2d43c86c-76a9-4566-a107-09d25d56fad3',
        username: 'etienne',
